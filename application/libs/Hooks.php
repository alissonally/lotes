...
if (!class_exists('Hooks')){
  ...
  */
  class Hooks
  {
    ...
    {
      Hooks::$filters = array();
      Hooks::$merged_filters = array();
      Hooks::$actions = array();
      Hooks::$current_filter = array();
    }
    ...
    static function add_filter($tag, $function_to_add, $priority = 10, $accepted_args = 1) {
      ...
      Hooks::$filters[$tag][$priority][$idx] = array('function' => $function_to_add, 'accepted_args' => $accepted_args);
      unset( Hooks::$merged_filters[ $tag ] );
      return true;
    }
    /**
    ...
    static function remove_filter( $tag, $function_to_remove, $priority = 10 ) {
      ...
      $r = isset(Hooks::$filters[$tag][$priority][$function_to_remove]);

      if ( true === $r) {
        unset(Hooks::$filters[$tag][$priority][$function_to_remove]);
        if ( empty(Hooks::$filters[$tag][$priority]) )
          unset(Hooks::$filters[$tag][$priority]);
        unset(Hooks::$merged_filters[$tag]);
      }
      return $r;
    }
    ...
    static function remove_all_filters($tag, $priority = false) {
      if( isset(Hooks::$filters[$tag]) ) {
        if( false !== $priority && isset(Hooks::$filters[$tag][$priority]) )
          unset(Hooks::$filters[$tag][$priority]);
        else
          unset(Hooks::$filters[$tag]);
      }
    ...
    static function has_filter($tag, $function_to_check = false) {
      ...
      foreach ( (array) array_keys(Hooks::$filters[$tag]) as $priority ) {
        if ( isset(Hooks::$filters[$tag][$priority][$idx]) )
          return $priority;
      }
      return false;
    }
    ...
    static function did_action($tag) {
      ...
      if ( ! isset( Hooks::$actions ) || ! isset( $this->actions[$tag] ) )
        return 0;

      return Hooks::$actions[$tag];
    }
    ...
    static function current_filter() {
      return end( Hooks::$current_filter );
    }
    /**
    ...
    function doing_filter( $filter = null ) {
      if ( null === $filter ) {
        return ! empty( Hooks::$current_filter );
      } 
      return in_array( $filter, Hooks::$current_filter );
    }
    ...
    function doing_action( $action = null ) {
      return Hooks::doing_filter( $action );
    }
     ...
     */
    static function _filter_build_unique_id($tag, $function, $priority) {
      ...
      if (is_object($function[0]) ) {
        ...
        if ( function_exists('spl_object_hash') ) {
          ...
          if ( !isset($function[0]->filter_id) ) {
            if ( false === $priority )
              return false;
            $obj_idx .= isset(Hooks::$filters[$tag][$priority]) ? count((array)Hooks::$filters[$tag][$priority]) : $filter_id_count;
     ...
     */
    static function __call_all_hook($args) {
      reset( Hooks::$filters['all'] );
      do {
        foreach( (array) current(Hooks::$filters['all']) as $the_ )
          if ( !is_null($the_['function']) )
            call_user_func_array($the_['function'], $args);

      } while ( next(Hooks::$filters['all']) !== false );
    }
  }//end class
}//end if